...
const LoginEmptyState = () => {
  ...
  const handleLogin = async () => {
    try {
      const response = await fetch(`${config.ipAddress}/api/login`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        // Check the redirect key to determine where to navigate
        if (data.redirect === 'home') {
          navigation.navigate('home', { user_id: data.user_id });
        } else if (data.redirect === 'data') {
          navigation.navigate('data', { user_id: data.user_id });
        }
      } else {
